...
#| results: asis
#| eval: !expr 'file.exists(get_model_outputpath(mode = "modelresults", current_sp = current_sp))'

df_fairness_list <- list()

# Walk over the variables
for (i in c(sensitive_labels)) {
  fairness_output_path       <- get_model_outputpath(mode = "fairness", group = i, current_sp = current_sp)
