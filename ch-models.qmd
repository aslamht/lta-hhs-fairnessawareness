...
De variabelen die we hebben geselecteerd voor analyse is op basis van standaardgegevens van een popoluatie (leeftijd en geslacht), eerdere analsyes op voorspelkracht voor `r tolower(cfg$model_settings$succes)` [@Bakker.2022], of omdat een kenmerk volgens Europese normen geldt als een sensitief kenmerk voor discriminatie [@handboek.2018].
...
**Opleiding**: `r current_sp$INS_Faculteit` \| `r get_sp_name_syn(current_sp$INS_Opleidingsnaam_huidig)` (`r current_sp$INS_Opleiding`), `r current_sp$INS_Opleidingsvorm`, eerstejaars - **`r cfg$model_settings$succes `**
...
if (cfg$model_settings$use_synthetic_data) {
...
df_sp_enrollments <- df_sp_enrollments |> 
  ...
  mutate_retention(cfg$model_settings$succes) |>
...
df_sp_enrollments <- df_sp_enrollments |> 
  distinct() |>
  select(sort(names(df_sp_enrollments)))
...
#| out-width: 80%
knitr::include_graphics(here::here("metadata/images", "prognosemodel-dataset-lta-hhs.png"))
```
...
splits      <- rsample::initial_validation_split(df_sp_enrollments,
                                        strata = Retentie,
                                        prop = c(0.6, 0.2))

# Create three sets: a training set, a test set and a validation set
df_retention_train      <- rsample::training(splits)
df_retention_test       <- rsample::testing(splits)
df_retention_validation <- rsample::validation_set(splits)

# Create a resample set based on 10 folds (default)
df_retention_resamples  <- rsample::vfold_cv(df_retention_train, strata = Retentie)
```
...
lr_recipe <- 
  recipes::recipe(Retentie ~ ., data = df_retention_train) |>  
  recipes::update_role(ID, new_role = "ID") |>           # Set the student ID as an ID variable
  recipes::step_rm(ID, Collegejaar) |>                   # Remove ID and college year from the model
  recipes::step_unknown(Studiekeuzeprofiel, 
               new_level = "Onbekend skp") |>   # Add unknown skp
  recipes::step_dummy(all_nominal_predictors()) |>       # Create dummy variables from categorical variables
  recipes::step_zv(all_predictors()) |>                  # Remove zero values
  recipes::step_normalize(all_numeric_predictors())      # Center and scale numeric variables
...
lr_res <- 
  lr_workflow |> 
  tune::tune_grid(df_retention_validation,
            grid = lr_reg_grid,
...
lr_auc <- 
  ...
  yardstick::roc_curve(Retentie, .pred_FALSE) |> 
...
lr_auc_highest   <-
  lr_res |>
  tune::collect_predictions(parameters = lr_best) |> 
  yardstick::roc_auc(Retentie, .pred_FALSE)
...
best_model_text <- df_model_results |> 
  filter(best == FALSE) |> 
  glue::glue_data("Het {model} model heeft een AUC van {round(auc, 4)}. ") |> 
  stringr::str_flatten()
...
knitr::include_graphics(here::here("metadata/images", "basemodel-lift.png"))
...
confusion_matrix <- last_fit |>
  tune::collect_predictions() |>
  yardstick::conf_mat(truth = Retentie, estimate = .pred_class) 
...
knitr::include_graphics(here::here("metadata/images", "confusion-matrix-retention-lta-hhs.png"))
