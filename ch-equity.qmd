...
df_sp_enrollments <- rio::import(data_outputpath, trust = TRUE) |>
  mutate(across(all_of(names(levels)), ~ factor(.x, levels = levels[[cur_column()]]))) |>
...
source("scripts/load-dfpersona.R")

df_persona_all <- load_dfpersona(df_sp_enrollments, sensitive_labels)
...
::: {.content-visible when-profile="advanced-report"}
{{< include scripts/qmd/advanced/confusion-matrix-tabset.qmd >}}
:::
...
for (group in c(sensitive_labels)) {
  ...
  density_plot <- suppressWarnings(
    get_density_plot(fairness_object, group = group, current_sp = current_sp) 
  ) 

  # Save the plot
  plot_name <- glue::glue("density_plot_{tolower(group)}")
  ...
  plot <- glue::glue("![Verdeling en dichtheid van {tolower(cfg$research_settings$succes_label)} ",
...
::: {.content-visible when-profile="advanced-report"}
{{< include scripts/qmd/advanced/bias-metrics-reasons.qmd >}}
:::
...
for (group in c(sensitive_labels)) {
  ...
  fairness_output_path  <- get_model_outputpath(mode = "fairness", current_sp = current_sp, group = group)
  df_fairness_check_data <- get_df_fairness_check_data(fairness_object[["fairness_check_data"]],
                                                       group = group, current_sp)
